...
router.get("/", (req, res) => {
  ...
  DB.all(
    "SELECT * FROM shifts WHERE organisation_id = ?",
...
router.post("/", (req, res) => {
  ...
  const { userId, start, finish, breakLength, organisationId } = req.body;
  ...
  DB.get(
    ...
    .then(() =>
      DB.run(
        "INSERT INTO shifts (user_id, start, finish, break_length, organisation_id) VALUES (?, ?, ?, ?, ?)",
        userId,
        start,
        finish,
        breakLength,
